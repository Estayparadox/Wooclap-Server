...
function createUsers() {
  db.run('create table if not exists `users` (' + '`id` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE,' + '`name` VARCHAR(255) NOT NULL,' + '`email_address` VARCHAR(255) NOT NULL);');
}

function createStats() {
  db.run('create table if not exists `stats` (' + '`id` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE,' + '`nb_collected_answers` INTEGER NOT NULL,' + '`nb_created_events` INTEGER NOT NULL,' + '`nb_created_questions` INTEGER NOT NULL,' + '`user_id` INTEGER REFERENCES `users` (`id`) ON DELETE SET NULL ON UPDATE CASCADE);');
}

function createUserHasAlreadySubmit() {
  db.run('create table if not exists `userHasAlreadySubmit` (' + '`id` INTEGER PRIMARY KEY AUTOINCREMENT,' + '`name` VARCHAR(255) NOT NULL,' + '`organisation` VARCHAR(255) NOT NULL,' + '`job_title` VARCHAR(255) NOT NULL,' + '`email_address` VARCHAR(255) NOT NULL);');
}

function createOrganisations() {
  db.run('create table if not exists `organisations` (' + '`id` INTEGER PRIMARY KEY AUTOINCREMENT,' + '`name` VARCHAR(255) NOT NULL,' + '`email_domain` VARCHAR(255) NOT NULL,' + '`is_client` INTEGER NOT NULL,' + 'UNIQUE(`id`, `name`, `email_domain`));');
}
...
app.post("/api/send-custom-mail", function (req, res, next) {
  ...
  var concernedUsers = req.body.concernedUsers;
...
app.patch("/api/organisation/:id", function (req, res, next) {
  ...
  console.log("PATCH /organisation/".concat(req.params.id));
});
